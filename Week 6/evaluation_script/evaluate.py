...
if __name__ == '__main__': 
    ...
    full_annotations = pd.read_csv(r'c5/week6/evaluation_script/test_set_age_labels.csv',sep = ',')
    average = True
    if average:
        full_annotations = full_annotations.drop_duplicates(subset='UserID', keep='first')
        full_annotations['VideoName'] = full_annotations['UserID'] + '.mp4'
        full_annotations = full_annotations.reset_index(drop=True)
    
    pretictions_with_gt = pd.read_csv(r'/ghome/group08/c5/week6/outputs_task_g/predictions_test_set_multimodal_average_mask_reweighting.csv',sep = ',')
