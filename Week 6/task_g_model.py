...
class AudioModel(nn.Module):
    ...
    def __init__(self, device):
        ...
        attention = "flash_attention_2" if device == "cuda" else "eager"

        model_id = "distil-whisper/distil-large-v3"

        self.processor = AutoProcessor.from_pretrained(model_id)
        self.audio_encoder = AutoModelForSpeechSeq2Seq.from_pretrained(model_id, torch_dtype=self.torch_dtype, low_cpu_mem_usage=True, use_safetensors=True, attn_implementation=attention, device_map=device)
...
class MultiModalModel(nn.Module):
    def __init__(self, device, num_classes=7, replace_token='', hidden_dim_audio=1280, hidden_dim_text=768, hidden_dim_vision=768):
        ...
        self.fc = nn.Linear(hidden_dim_audio + hidden_dim_text + hidden_dim_vision, 1024)
        self.dropout = nn.Dropout(0.3)
        self.fc2 = nn.Linear(1024, num_classes)

    def forward(self, image, audio, text):
        vision_output = self.vision_model(image)
        # audio_output = self.attentive_pooling(self.audio_model(audio))
        ...
        x = torch.cat([vision_output, audio_output, text_output], dim=1)
