...
class SqueezeBlock(nn.Module):
    def __init__(self, input_channels, squeeze_planes, expand1x1_planes, expand3x3_planes):
        super(SqueezeBlock, self).__init__()
        self.squeeze = nn.Conv2d(in_channels=input_channels, out_channels=squeeze_planes, kernel_size=1)
...
class Model(nn.Module):
    def __init__(self, num_classes=8):
        ...
        self.squeeze_block1 = SqueezeBlock(64, 64, 128, 128)
        self.squeeze_block2 = SqueezeBlock(256, 64, 256, 256)

        self.batch_norm1 = nn.BatchNorm2d(num_features=64)
        self.batch_norm2 = nn.BatchNorm2d(num_features=256)
        self.batch_norm3 = nn.BatchNorm2d(num_features=512)

        self.final_conv = nn.Conv2d(in_channels=512, out_channels=num_classes, kernel_size=3, padding=1)
